...
public class ProductSelectAllHighAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 모든 상품 데이터들을 가격순으로 pdatas 라는 이름으로 저장
//		ProductDAO dao = ProductDAO.getInstance();
//		ArrayList<ProductVO> pdatas = dao.selectAllHigh();
//		request.setAttribute("pcode", request.getParameter("pcode"));
//		request.setAttribute("pdatas", pdatas);
//		
		
		// pcode가 존재시 pcode에 해당하는 상품들만, 없을시 모든 상품 데이터들을 가격순으로 pdatas 라는 이름으로 저장
		ProductDAO dao = ProductDAO.getInstance();
		...
		if(request.getParameter("pcode") == null) {
			...
			ArrayList<ProductVO> pdatas = dao.selectAllHPPcode(vo);
			request.setAttribute("pdatas", pdatas);
			request.setAttribute("pcode", request.getParameter("pcode"));
		}
